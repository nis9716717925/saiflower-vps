...
// 2. GET FLOWER DATA (By Slug or ID)
$flower = null;
if (isset($_GET['slug'])) {
    $slug = $_GET['slug'];
    $stmt = $conn->prepare("SELECT * FROM flowers WHERE slug = ?");
...
if (!$flower) { header("Location: flowers.php"); exit; }

enforce_canonical_product_url('flower', $flower);
set_page_canonical_url(get_product_canonical_url('flower', $flower));

$id = $flower['id'];
$in_stock = (bool)$flower['in_stock']; // Stock Status Logic

// Wishlist Check
$is_wishlisted = false;
if(isset($_SESSION['customer_id'])) {
    $uid = $_SESSION['customer_id'];
    $wQ = mysqli_query($conn, "SELECT id FROM wishlist WHERE user_id = $uid AND product_id = $id AND type = 'flower'");
    if($wQ && mysqli_num_rows($wQ) > 0) $is_wishlisted = true;
}

$page_title = $flower['meta_title'] ?: $flower['name'] . ' | Sai Flowers';
$meta_desc = $flower['meta_description'] ?: mb_strimwidth(strip_tags($flower['description']), 0, 160, "...");

if (($settings['maintenance_mode'] ?? 0) == 1) { header("Location: maintenance.php"); exit; }
$finalImg = get_image_url($flower['image'], 'flowers');
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    // Global Flower FAQs for Schema
    $globalFaqs = [
        ["question" => "Do you offer same-day flower delivery?", "answer" => "Yes, we offer same-day flower delivery in selected areas for orders placed before the cut-off time."],
        ["question" => "Can I schedule flower delivery for a specific date and time?", "answer" => "Yes, you can choose your preferred delivery date during checkout. Time-slot delivery may also be available in selected locations."],
        ["question" => "Are the flowers fresh and hand-arranged?", "answer" => "Absolutely! All bouquets and floral arrangements are prepared using fresh flowers by experienced florists."],
        ["question" => "Can I add a personalized message with my order?", "answer" => "Yes, you can include a custom message card with your flowers at no extra cost."],
        ["question" => "Do you deliver flowers at midnight?", "answer" => "Yes, we provide midnight flower delivery for birthdays, anniversaries, and special occasions in many cities."],
        ["question" => "What happens if the recipient is unavailable during delivery?", "answer" => "Our delivery team will contact the recipient or follow the delivery instructions provided at checkout."],
        ["question" => "Can I customize the bouquet or flower arrangement?", "answer" => "Yes, customization options are available for many products. You can contact our support team for special requests."],
        ["question" => "Which payment methods do you accept?", "answer" => "We accept UPI, credit/debit cards, net banking, wallets, and other secure online payment methods."],
        ["question" => "Do you offer combo gifts with flowers?", "answer" => "Yes, we offer flower combos with cakes, chocolates, teddy bears, plants, and personalized gifts."],
        ["question" => "How can I track my flower delivery order?", "answer" => "Once your order is confirmed, you will receive order updates and tracking details via email or WhatsApp/SMS."]
    ];
    $productFaqs = !empty($flower['faqs']) ? json_decode($flower['faqs'], true) : [];
    if (!is_array($productFaqs)) $productFaqs = [];
    $allFaqsForSchema = array_merge($globalFaqs, $productFaqs);
    
    $flowerForSchema = $flower;
    $flowerForSchema['faqs'] = json_encode($allFaqsForSchema);

    echo generate_product_json_ld($flowerForSchema, 'flower');
    echo generate_simple_breadcrumb_json_ld([
        ['name' => 'Flowers', 'item' => 'flowers.php'],
        ['name' => $flower['name'], 'item' => product_url(['type' => 'flower', 'slug' => $flower['slug'] ?? '', 'id' => $flower['id']])]
    ]);
    ?>
    <meta charset="UTF-8">
...
<div class="pd-container pd-page">
    <nav class="pd-breadcrumb" aria-label="Breadcrumb">
        ...
        <span aria-hidden="true"> / </span>
        <span class="pd-bc-current"><?= htmlspecialchars($flower['name']) ?></span>
    </nav>
    ...
    <div class="pd-grid">
        <div class="pd-gallery-col pd-gallery-sticky">
            <div class="pd-media-wrap is-zoomable" id="mediaContainer">
                ...
                <button type="button" onclick="toggleWishlist(this, <?= $id ?>, 'flower')" class="absolute top-4 right-4 w-10 h-10 bg-white/90 backdrop-blur rounded-full flex items-center justify-center shadow-md transition-colors z-20 hover:bg-red-50 <?= $is_wishlisted ? 'text-red-500' : 'text-slate-400' ?>" aria-label="Wishlist">
                    <span class="material-icons-outlined text-2xl"><?= $is_wishlisted ? 'favorite' : 'favorite_border' ?></span>
                </button>
                <?php 
                if(!empty($flower['model_3d'])): ?>
                    <model-viewer id="mainModel" 
                        src="/<?= htmlspecialchars($flower['model_3d']) ?>" 
                ...
                <img src="<?= $finalImg ?>" id="mainView" width="800" height="800" fetchpriority="high"
                     class="<?= !empty($flower['model_3d']) ? 'hidden-media' : 'visible-media' ?>" 
                     alt="<?= htmlspecialchars($flower['image_alt'] ?? $flower['name']) ?>" 
                     onerror="this.src='https://images.unsplash.com/photo-1526047932273-341f2a7631f9'">
            </div>
            ...
            <div class="pd-thumbs" role="list">
                ...
                // 1. 3D Model Thumbnail (Limit 1)
                if(!empty($flower['model_3d'])): ?>
                <div role="listitem" class="pd-thumb <?= !empty($flower['model_3d']) ? 'is-active' : '' ?> w-20 h-20 rounded-xl overflow-hidden cursor-pointer border-2 border-primary flex flex-col items-center justify-center bg-slate-50 shrink-0" 
                ...
                <!-- 2. Main Image Thumbnail (Limit 1) -->
                <div role="listitem" class="pd-thumb <?= empty($flower['model_3d']) ? 'is-active' : '' ?> w-20 h-20 rounded-xl overflow-hidden cursor-pointer border-2 <?= empty($flower['model_3d']) ? 'border-primary' : 'border-transparent' ?> hover:border-slate-300 shrink-0" 
                     onclick="switchMedia('image', this, '<?= $finalImg ?>')">
                    <img src="<?= $finalImg ?>" class="w-full h-full object-cover" width="80" height="80" loading="lazy" decoding="async" alt="<?= htmlspecialchars($flower['name']) ?>">
                </div>

                <?php
                // 3. Gallery Images (Limit 2 Total)
                
                // NEW: Check images_gallery (JSON array)
                if ($galleryCount < $maxGallery && !empty($flower['images_gallery'])) {
                    $gallery = json_decode($flower['images_gallery'], true);
                    if (is_array($gallery)) {
                        foreach ($gallery as $gPathRaw) {
                            if ($galleryCount >= $maxGallery) break;
                            $gPath = get_image_url($gPathRaw, 'flowers');
                            ?>
                            <div role="listitem" class="pd-thumb w-20 h-20 rounded-xl overflow-hidden cursor-pointer border-2 border-transparent hover:border-slate-200 shrink-0" 
                                 onclick="switchMedia('image', this, '<?= $gPath ?>')">
                                <img src="<?= $gPath ?>" class="w-full h-full object-cover" width="80" height="80" loading="lazy" decoding="async" alt="<?= htmlspecialchars($flower['name']) ?> gallery">
                            </div>
                            <?php
                            $galleryCount++;
                        }
                ...
                // LEGACY: Check flower_images table
                if ($galleryCount < $maxGallery) {
                    $galQ = mysqli_query($conn, "SELECT image_path FROM flower_images WHERE flower_id = $id");
                    while($g = mysqli_fetch_assoc($galQ)): 
                        if ($galleryCount >= $maxGallery) break;
                        $gPath = get_image_url($g['image_path'], 'flowers');
                    ?>
                    <div role="listitem" class="pd-thumb w-20 h-20 rounded-xl overflow-hidden cursor-pointer border-2 border-transparent hover:border-slate-200 shrink-0"
                         onclick="switchMedia('image', this, '<?= $gPath ?>')">
                        <img src="<?= $gPath ?>" class="w-full h-full object-cover" width="80" height="80" loading="lazy" decoding="async" alt="<?= htmlspecialchars($flower['name']) ?> gallery">
        ...
        <div class="pd-buy-col pd-buy-sticky product-details">
            <div class="pd-badge-row">
                <?php if ($in_stock): ?>
                    <span class="pd-badge">Signature selection</span>
                    <?php if(!empty($flower['original_price']) && $flower['original_price'] > $flower['price']): ?>
            ...
            <h1 class="pd-title"><?= htmlspecialchars($flower['name']) ?></h1>
            ...
            <div class="pd-rating-row">
                ...
                <span class="pd-rating-val"><?= number_format($flower['rating'] ?? 5.0, 1) ?></span>
                <span class="pd-rating-meta">120+ verified reviews</span>
                <span class="pd-rating-highlight">Customer favourite in Delhi</span>
            </div>

            <?php if(!empty($flower['tag'])): 
                $tags = array_filter(explode(',', $flower['tag']));
                if(count($tags) > 0):
            ?>
            ...
            <div class="pd-delivery-chips">
                <?php if(!isset($flower['delivery_sameday']) || $flower['delivery_sameday'] == 1): ?>
                ...
                <?php elseif(isset($flower['delivery_nextday']) && $flower['delivery_nextday'] == 1): ?>
            ...
            <div class="pd-price-block price-display" id="priceDisplayBlock">
                ...
                <?php elseif(!empty($flower['original_price']) && $flower['original_price'] > $flower['price']): 
                    $surgedP = apply_surge_pricing($flower['price'], 'flower');
                    $disc = round(($flower['original_price'] - $surgedP) / $flower['original_price'] * 100); ?>
                    <span class="pd-price-old price-old">₹<?= number_format($flower['original_price']) ?></span>
                ...
                <?php else: ?>
                    <span id="priceText" class="pd-price-current price-current">₹<?= number_format(apply_surge_pricing($flower['price'], 'flower')) ?></span>
                <?php endif; ?>
            </div>
    ...
    <div id="descShort" class="pd-desc">
        <?= nl2br(htmlspecialchars(mb_strimwidth($flower['description'], 0, 220, "..."))) ?>
    </div>

    <div id="descFull" class="pd-desc hidden">
        <?= nl2br(htmlspecialchars($flower['description'])) ?>
    </div>

    <?php if(strlen($flower['description']) > 220): ?>
    ...
    <?php endif; ?>
            ...
            <div class="pd-buy-card buy-action-card <?= !$in_stock ? 'opacity-60 grayscale' : '' ?>">
                <form id="productPurchaseForm" action="/cart.php" method="POST">
                    ...
                    <input type="hidden" name="name" value="<?= htmlspecialchars($flower['name']) ?>">
                    <input type="hidden" name="price" id="formPrice" value="<?= apply_surge_pricing($flower['price'], 'flower') ?>">
                    ...
                    <div class="flex flex-col gap-6">
                        <?php 
                        $vars = mysqli_query($conn, "SELECT * FROM flower_variants WHERE flower_id = $id");
                        if(mysqli_num_rows($vars) > 0): ?>
                        <div>
                            <label class="pd-variant-label">Choose option</label>
                            <div class="pd-variant-btns" id="variantButtons">
                                <button type="button" class="variant-btn pd-variant-btn active" data-price="<?= apply_surge_pricing($flower['price'], 'flower') ?>" data-orig-price="<?= $flower['original_price'] ?? '' ?>" onclick="setVariant(this)" <?= !$in_stock ? 'disabled' : '' ?>>Standard</button>
                                <?php while($v = mysqli_fetch_assoc($vars)): ?>
                                <button type="button" class="variant-btn pd-variant-btn" data-price="<?= apply_surge_pricing($v['price'], 'flower') ?>" data-orig-price="<?= $v['original_price'] ?? '' ?>" onclick="setVariant(this)" <?= !$in_stock ? 'disabled' : '' ?>><?= htmlspecialchars($v['name']) ?></button>
                                <?php endwhile; ?>
                            </div>
                        </div>
                        <?php endif; ?>
    ...
    <section class="content-visibility-auto">
        ...
        <div class="swiper swiper-recommendations">
            <div class="swiper-wrapper">
                <?php 
                $recQ = mysqli_query($conn, "SELECT * FROM flowers WHERE id != $id AND status = 1 ORDER BY RAND() LIMIT 10");
                while($rec = mysqli_fetch_assoc($recQ)):
                    $recImg = (strpos($rec['image'], 'uploads/') === 0) ? "/" . $rec['image'] : "/uploads/" . $rec['image'];
$recLink = product_url(['type' => 'flower', 'slug' => $rec['slug'] ?? '', 'id' => $rec['id']]);                ?>
                <div class="swiper-slide">
                    <a href="<?= $recLink ?>" class="pd-rec-card group block rounded-3xl overflow-hidden">
                        <div class="aspect-[4/5] overflow-hidden">
                            <img src="<?= $recImg ?>" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" width="400" height="500" loading="lazy" decoding="async" alt="<?= htmlspecialchars($rec['name']) ?> flower arrangement" onerror="this.src='https://images.unsplash.com/photo-1526047932273-341f2a7631f9'">
                        </div>
                        <div class="p-5">
                            <h3 class="font-bold text-slate-800 text-sm truncate"><?= htmlspecialchars($rec['name']) ?></h3>
                            <div class="flex justify-between items-center mt-2">
                                <span class="text-primary font-bold">₹<?= number_format(apply_surge_pricing($rec['price'], 'flower')) ?></span>
    ...
    <?php 
    // Reuse the already defined $globalFaqs and $productFaqs from the head
    $allFaqs = array_merge($globalFaqs, $productFaqs);
