...
<head>
    ...
    echo generate_simple_breadcrumb_json_ld([['name' => 'Help Center', 'item' => 'faq.php']]);
    
    // Generate FAQPage schema from the database
    $faqSchemaRes = mysqli_query($conn, "SELECT question, answer FROM faqs WHERE status = 1 LIMIT 50");
    $faqEntities = [];
    if ($faqSchemaRes) {
        while ($fs = mysqli_fetch_assoc($faqSchemaRes)) {
            $faqEntities[] = [
                '@type' => 'Question',
                'name' => htmlspecialchars_decode($fs['question']),
                'acceptedAnswer' => [
                    '@type' => 'Answer',
                    'text' => strip_tags(htmlspecialchars_decode($fs['answer']))
                ]
            ];
        }
    }
