...
// 2. GET CAKE DATA (By Slug or ID)
$cake = null;
if (isset($_GET['slug'])) {
    $slug = $_GET['slug'];
    $stmt = $conn->prepare("SELECT * FROM cakes WHERE slug = ?");
...
enforce_canonical_product_url('cake', $cake);
set_page_canonical_url(get_product_canonical_url('cake', $cake));

$id = $cake['id'];
$in_stock = (bool)$cake['in_stock']; // Stock Status Logic
$page_title = $cake['meta_title'] ?: $cake['name'] . ' | Sai Cakes';
$meta_desc = $cake['meta_description'] ?: mb_strimwidth(strip_tags($cake['description']), 0, 160, "...");
...
if(isset($_SESSION['customer_id'])) {
    ...
    $wQ = mysqli_query($conn, "SELECT id FROM wishlist WHERE user_id = $uid AND product_id = $id AND type = 'cake'");
    if($wQ && mysqli_num_rows($wQ) > 0) $is_wishlisted = true;
}
$finalImg = get_image_url($cake['image'] ?? '', 'cakes');
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    echo generate_product_json_ld($cake, 'cake');
    echo generate_simple_breadcrumb_json_ld([
        ['name' => 'Cakes', 'item' => 'cakes.php'],
        ['name' => $cake['name'], 'item' => product_url(['type' => 'cake', 'slug' => $cake['slug'] ?? '', 'id' => $cake['id']])]
    ]);
    ?>
    <meta charset="UTF-8">
...
<div class="pd-container pd-page">
    <nav class="pd-breadcrumb" aria-label="Breadcrumb">
        ...
        <span aria-hidden="true"> / </span>
        <span class="pd-bc-current"><?= htmlspecialchars($cake['name']) ?></span>
    </nav>
    ...
    <div class="pd-grid">
        <div class="pd-gallery-col pd-gallery-sticky">
            <div class="pd-media-wrap is-zoomable" id="mediaContainer">
                ...
                <button type="button" onclick="toggleWishlist(this, <?= $id ?>, 'cake')" class="absolute top-4 right-4 w-10 h-10 bg-white/90 backdrop-blur rounded-full flex items-center justify-center shadow-md transition-colors z-20 hover:bg-red-50 <?= $is_wishlisted ? 'text-red-500' : 'text-slate-400' ?>" aria-label="Wishlist">
                    <span class="material-icons-outlined text-2xl"><?= $is_wishlisted ? 'favorite' : 'favorite_border' ?></span>
                </button>
                <?php 
                if(!empty($cake['model_3d'])): ?>
                    <model-viewer id="mainModel" 
                        src="/<?= htmlspecialchars($cake['model_3d']) ?>" 
                ...
                <img src="<?= $finalImg ?>" id="mainView" width="800" height="800" fetchpriority="high"
                     class="<?= !empty($cake['model_3d']) ? 'hidden-media' : 'visible-media' ?>" 
                     alt="<?= htmlspecialchars($cake['image_alt'] ?? $cake['name']) ?>" 
                     onerror="this.src='https://placehold.co/600x600?text=Cake'">
            </div>
            ...
            <div class="pd-thumbs" role="list">
                ...
                if(!empty($cake['model_3d'])): ?>
                <div role="listitem" class="pd-thumb <?= !empty($cake['model_3d']) ? 'is-active' : '' ?> w-20 h-20 rounded-xl overflow-hidden cursor-pointer border-2 border-primary flex flex-col items-center justify-center bg-slate-50 shrink-0" 
                ...
                <div role="listitem" class="pd-thumb <?= empty($cake['model_3d']) ? 'is-active' : '' ?> w-20 h-20 rounded-xl overflow-hidden cursor-pointer border-2 <?= empty($cake['model_3d']) ? 'border-primary' : 'border-transparent' ?> hover:border-slate-300 shrink-0" 
                     onclick="switchMedia('image', this, '<?= $finalImg ?>')">
                    <img src="<?= $finalImg ?>" class="w-full h-full object-cover" width="80" height="80" alt="<?= htmlspecialchars($cake['name']) ?>">
                </div>
                <?php
                if($galleryCount < $maxGallery && !empty($cake['images_gallery'])) {
                    $gallery = json_decode($cake['images_gallery'], true);
                    if($gallery) {
                        foreach($gallery as $gPath):
                            if ($galleryCount >= $maxGallery) break;
                            $fullGPath = get_image_url($gPath, 'cakes');
                ?>
                <div role="listitem" class="pd-thumb w-20 h-20 rounded-xl overflow-hidden cursor-pointer border-2 border-transparent hover:border-slate-200 shrink-0" 
                     onclick="switchMedia('image', this, '<?= $fullGPath ?>')">
                    <img src="<?= $fullGPath ?>" class="w-full h-full object-cover" width="80" height="80" loading="lazy" alt="<?= htmlspecialchars($cake['name']) ?> gallery">
                </div>
                <?php 
                            $galleryCount++;
                        endforeach;
        ...
        <div class="pd-buy-col pd-buy-sticky product-details">
            <div class="pd-badge-row">
                <?php if ($in_stock): ?>
                    <span class="pd-badge">Artisan cake</span>
                    <?php if(!empty($cake['original_price']) && $cake['original_price'] > $cake['price']): ?>
            ...
            <h1 class="pd-title"><?= htmlspecialchars($cake['name']) ?></h1>
            ...
            <div class="pd-rating-row">
                ...
                <span class="pd-rating-val"><?= number_format($cake['rating'] ?? 5.0, 1) ?></span>
                <span class="pd-rating-meta">120+ verified reviews</span>
                <span class="pd-rating-highlight">Bestselling cakes</span>
            </div>

            <?php if(!empty($cake['tag'])): 
                $tags = array_filter(explode(',', $cake['tag']));
                if(count($tags) > 0):
            ?>
            ...
            <div class="pd-delivery-chips">
                <?php if(!isset($cake['delivery_sameday']) || $cake['delivery_sameday'] == 1): ?>
                ...
                <?php elseif(isset($cake['delivery_nextday']) && $cake['delivery_nextday'] == 1): ?>
            ...
            <div class="pd-price-block price-display" id="priceDisplayBlock">
                ...
                <?php elseif(!empty($cake['original_price']) && $cake['original_price'] > $cake['price']): 
                    $surgedP = apply_surge_pricing($cake['price'], 'cake');
                    $disc = round(($cake['original_price'] - $surgedP) / $cake['original_price'] * 100); ?>
                    <span class="pd-price-old price-old">₹<?= number_format($cake['original_price']) ?></span>
                ...
                <?php else: ?>
                    <span id="priceText" class="pd-price-current price-current">₹<?= number_format(apply_surge_pricing($cake['price'], 'cake')) ?></span>
                <?php endif; ?>
            </div>

            <?php if ($in_stock): ?>
            <div class="pd-stock-row">
                <span class="pd-stock-ok"><i class="fas fa-circle-check" aria-hidden="true"></i> In stock — fresh bake schedule</span>
                <span class="pd-urgency">Order early for fixed-slot delivery</span>
            </div>
            <?php endif; ?>

            <div class="mb-2 pd-desc"><?= nl2br(htmlspecialchars(mb_strimwidth($cake['description'], 0, 220, "..."))) ?></div>
            ...
            <div class="pd-buy-card buy-action-card <?= !$in_stock ? 'opacity-60 grayscale' : '' ?>">
                <form id="productPurchaseForm" action="/cart.php" method="POST">
                    ...
                    <input type="hidden" name="name" value="<?= htmlspecialchars($cake['name']) ?>">
                    <input type="hidden" name="price" id="formPrice" value="<?= apply_surge_pricing($cake['price'], 'cake') ?>">
                    ...
                    <div class="flex flex-col gap-6">
                        <?php
                        $vars = mysqli_query($conn, "SELECT * FROM cake_variants WHERE cake_id = $id");
                        if(mysqli_num_rows($vars) > 0): ?>
                        <div>
                            <label class="pd-variant-label">Choose option</label>
                            <div class="pd-variant-btns" id="variantButtons">
                                <button type="button" class="variant-btn pd-variant-btn active" data-price="<?= apply_surge_pricing($cake['price'], 'cake') ?>" data-orig-price="<?= $cake['original_price'] ?? '' ?>" onclick="setVariant(this)" <?= !$in_stock ? 'disabled' : '' ?>>Standard</button>
                                <?php while($v = mysqli_fetch_assoc($vars)): ?>
                                <button type="button" class="variant-btn pd-variant-btn" data-price="<?= apply_surge_pricing($v['price'], 'cake') ?>" data-orig-price="<?= $v['original_price'] ?? '' ?>" onclick="setVariant(this)" <?= !$in_stock ? 'disabled' : '' ?>><?= htmlspecialchars($v['name']) ?></button>
                                <?php endwhile; ?>
                            </div>
                        </div>
                        <?php endif; ?>
    ...
    <section class="content-visibility-auto">
        ...
        <div class="swiper swiper-recommendations">
            <div class="swiper-wrapper">
                <?php 
                $recQ = mysqli_query($conn, "SELECT * FROM cakes WHERE id != $id AND status = 1 ORDER BY RAND() LIMIT 10");
                while($rec = mysqli_fetch_assoc($recQ)):
                    $recImg = (strpos($rec['image'], 'uploads/') === 0) ? "/" . $rec['image'] : "/uploads/" . $rec['image'];
                    $recLink = product_url(['type' => 'cake', 'slug' => $rec['slug'] ?? '', 'id' => $rec['id']]);
                ?>
                <div class="swiper-slide">
                    <a href="<?= $recLink ?>" class="pd-rec-card group block rounded-3xl overflow-hidden">
                        <div class="aspect-[4/5] overflow-hidden">
                            <img src="<?= $recImg ?>" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" alt="<?= htmlspecialchars($rec['name']) ?> cake" onerror="this.src='https://placehold.co/400x500?text=Cake'">
                        </div>
                        <div class="p-5">
                            <h3 class="font-bold text-slate-800 text-sm truncate"><?= htmlspecialchars($rec['name']) ?></h3>
                            <div class="flex justify-between items-center mt-2">
                                <span class="text-primary font-bold">₹<?= number_format(apply_surge_pricing($rec['price'], 'cake')) ?></span>
    ...
    <?php if(!empty($cake['faqs'])): 
        $faqs = json_decode($cake['faqs'], true);
