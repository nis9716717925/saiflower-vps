...
$slug = $baseSlug;
$count = 1;
$stmt = $conn->prepare("SELECT id FROM gifts WHERE slug = ? AND id != ? LIMIT 1");
while(true){
...
try{
    ...
    if($uploadResult['success']){
        $fields[]="image=?"; $types.="s"; $params[]='uploads/gifts/'.$uploadResult['filename'];
    }
    ...
    /* ================= VARIANTS ================= */
    // Delete old variants
    $stmt = $conn->prepare("DELETE FROM gift_variants WHERE gift_id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $stmt->close();
