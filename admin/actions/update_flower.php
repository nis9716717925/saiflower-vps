<?php
// admin/actions/update_flower.php
...
if ($id <= 0) {
    $_SESSION['error'] = "Invalid flower ID.";
    header("Location: ../flowers.php");
    exit();
}
...
$stmt = $conn->prepare("SELECT id FROM flowers WHERE slug = ? AND id != ? LIMIT 1");
while(true){
...
return 'uploads/flowers/'.$newFilename;
}
return null;
}
...
$fields[]="category_ids=?"; $types.="s"; $params[]=$category_ids;
...
$fields[]="image=?"; $types.="s"; $params[]='uploads/flowers/'.$uploadResult['filename'];
}
...
$sql="UPDATE flowers SET ".implode(",",$fields)." WHERE id=?";
...
if(isset($_FILES['gallery']) && !empty($_FILES['gallery']['name'][0])){
    ...
    if(!empty($galleryPaths)){
        ...
        $stmt->bind_param("si", $imagesGallery, $id);
        $stmt->execute();
        $stmt->close();
    }
}
...
/* ================= VARIANTS ================= */
// Delete old variants
$stmt = $conn->prepare("DELETE FROM flower_variants WHERE flower_id = ?");
$stmt->bind_param("i", $id);
$stmt->execute();
$stmt->close();
...
$conn->commit();
$_SESSION['success']="Flower updated successfully.";
header("Location: ../flowers.php");
exit();
