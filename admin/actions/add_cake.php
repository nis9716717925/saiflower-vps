...
$stmt = $conn->prepare("SELECT id FROM cakes WHERE slug = ? LIMIT 1");
while(true){
...
try{
    ...
    $mainImage = 'uploads/cakes/'.$uploadResult['filename'];
    ...
    if(isset($_POST['variant_name']) && is_array($_POST['variant_name'])){
        ...
        for($i = 0; $i < count($vnames); $i++){
            ...
            if($vname !== '' && $vprice > 0){
                $vstmt->bind_param("isdd", $cakeId, $vname, $vorig_price, $vprice);
                $vstmt->execute();
            }
        }
        $vstmt->close();
    ...
    $_SESSION['success']="Cake added successfully";
