...
// 4. FETCH CAKES
$tableCheck = mysqli_query($conn, "SHOW TABLES LIKE 'cakes'");
if(mysqli_num_rows($tableCheck) == 0) {
    // Attempt to create table on the fly if missing
    $sql = "CREATE TABLE cakes LIKE flowers";
    if(mysqli_query($conn, $sql)) {
        echo "<div class='alert alert-success'>Table 'cakes' created successfully. Refreshing...</div>";
        echo "<script>setTimeout(() => window.location.reload(), 2000);</script>";
        exit;
    } else {
        die("Error: Table 'cakes' missing and could not be created: " . mysqli_error($conn));
    }
}
$query = "SELECT id, name, slug, price, original_price, image, status, in_stock, created_at FROM cakes ORDER BY id DESC";
$result = mysqli_query($conn, $query);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <?php $pageTitle = 'Manage Cakes'; include 'partials/head.php'; ?>
    ...
    <main class="admin-main">
        ...
        <div class="admin-header">
            <div class="header-title">
                <h2><i class="fa-solid fa-birthday-cake" style="color:var(--primary);"></i> Cake Inventory</h2>
                <p>Manage your cakes, pricing, and live availability.</p>
            </div>
            <a href="add-cake.php" class="btn-add">
                <i class="fa-solid fa-plus"></i> Add New Cake
            </a>
        </div>
            ...
            <div id="statusAlert" style="margin-bottom: 20px;">
                <?php if ($_GET['msg'] === 'restocked'): ?>
                    <div style="background:#e9fdf3; color:#10b981; padding:15px; border-radius:12px; font-weight:700; border:1px solid #d1fae5; display:flex; align-items:center; gap:10px;">
                        <i class="fas fa-check-circle"></i> Cake restocked successfully!
                    </div>
                <?php elseif ($_GET['msg'] === 'soldout'): ?>
                    <div style="background:#fff1f2; color:#f43f5e; padding:15px; border-radius:12px; font-weight:700; border:1px solid #ffe4e6; display:flex; align-items:center; gap:10px;">
                        <i class="fas fa-info-circle"></i> Cake marked as Sold Out.
                    </div>
                            ...
                            <tr>
                                ...
                                <td data-label="Image">
                                    <img src="/uploads/<?= htmlspecialchars($row['image']) ?>" 
                                         class="prod-img" 
                                         alt="cake"
                                         onerror="this.onerror=null; this.src='https://placehold.co/200x200/f8faf9/326e54?text=Cake'">
                                </td>
                                <td data-label="Product">
                                    <div style="font-weight:700; color: #333; font-size: 1rem;"><?= htmlspecialchars($row['name']) ?></div>
                                    <?php $liveLink = product_url(['type' => 'cake', 'slug' => $row['slug'] ?? '', 'id' => $row['id']]); ?>
                                ...
                                <td data-label="Stock">
                                    <?php if($row['in_stock']): ?>
                                        <a href="actions/toggle_stock.php?type=cakes&id=<?= $row['id'] ?>&stock=0&csrf_token=<?php echo csrf_token(); ?>" class="badge badge-success badge-clickable">
                                            <i class="fa-solid fa-circle-check"></i> In Stock
                                        </a>
                                    <?php else: ?>
                                        <a href="actions/toggle_stock.php?type=cakes&id=<?= $row['id'] ?>&stock=1&csrf_token=<?php echo csrf_token(); ?>" class="badge badge-danger badge-clickable">
                                ...
                                <td data-label="Actions">
                                    <div style="display: flex; gap: 10px; justify-content: flex-end;">
                                        <a href="edit-cake.php?id=<?= $row['id'] ?>" class="btn-action" title="Edit">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </a>
                                        <a href="actions/delete_cake.php?id=<?= $row['id'] ?>&csrf_token=<?php echo csrf_token(); ?>" 
                                           onclick="return confirm('Permanently delete this cake?')"
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="7" style="text-align:center; padding:100px; color:#bbb;">
                                <i class="fa-solid fa-birthday-cake" style="font-size: 3rem; display:block; margin-bottom:15px; opacity:0.3;"></i>
                                No cakes found in your inventory.
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
