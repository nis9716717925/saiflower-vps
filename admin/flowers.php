...
// 3. One-time Migration for Multiple Categories
...
if(isset($_GET['category']) && $_GET['category'] !== '') {
    ...
    $where = " WHERE category_ids LIKE '%,$cat_id,%'";
}
$query = "SELECT id, name, slug, price, original_price, image, status, in_stock, created_at FROM flowers $where ORDER BY id DESC";
$result = mysqli_query($conn, $query);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <?php $pageTitle = 'Manage Flowers'; include 'partials/head.php'; ?>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        @media (max-width: 992px) {
            ...
            .header-actions { flex-wrap: wrap; justify-content: stretch; }
            .header-actions > * { flex: 1; min-width: 140px; justify-content: center; }
            .btn-add { padding: 8px 16px; font-size: 0.8rem; height: 42px; width: 100%; }
    ...
    <main class="admin-main">
        ...
        <div class="admin-header">
            <div class="header-title">
                <h2><i class="fa-solid fa-leaf" style="color:var(--primary);"></i> Flower Inventory</h2>
                <p>Manage your bouquets, pricing, and live availability.</p>
            </div>
            
            <div class="header-actions" style="display:flex; gap:10px; align-items:center; flex-wrap: wrap;">
                <form action="flowers.php" method="GET" style="display:contents;">
                    <select name="category" onchange="this.form.submit()" style="padding:0 15px; border-radius:10px; border:1px solid #ddd; font-size:0.85rem; font-weight:600; height:42px; background:#fff; min-width:140px;">
                ...
                <a href="categories.php" class="btn-add" style="background:var(--accent); color:#fff; border-color:var(--accent); height:42px; padding:0 20px;">
                    <i class="fa-solid fa-list-check"></i> Manage Categories
                </a>
                <a href="add-flower.php" class="btn-add" style="height:42px; padding:0 20px;">
                    <i class="fa-solid fa-plus"></i> Add New Flower
                </a>
            </div>
        </div>
            ...
            <div id="statusAlert" style="margin-bottom: 20px;">
                <?php if ($_GET['msg'] === 'restocked'): ?>
                    <div style="background:#e9fdf3; color:#10b981; padding:15px; border-radius:12px; font-weight:700; border:1px solid #d1fae5; display:flex; align-items:center; gap:10px;">
                        <i class="fas fa-check-circle"></i> Flower restocked successfully!
                    </div>
                <?php elseif ($_GET['msg'] === 'soldout'): ?>
                    <div style="background:#fff1f2; color:#f43f5e; padding:15px; border-radius:12px; font-weight:700; border:1px solid #ffe4e6; display:flex; align-items:center; gap:10px;">
                        <i class="fas fa-info-circle"></i> Flower marked as Sold Out.
                    </div>
                        ...
                        <?php while ($row = mysqli_fetch_assoc($result)): ?>
                            <tr>
                                ...
                                <td data-label="Image">
                                    <?php 
                                    $img = $row['image'];
                                    // Fix: Check if already has uploads/ prefix
                                    $displayImg = strpos($img, 'uploads/') === 0 ? $img : 'uploads/' . $img;
                                    ?>
                                    <img src="/<?= htmlspecialchars($displayImg) ?>" 
                                         class="prod-img" 
                                         alt="flower"
                                         onerror="this.onerror=null; this.src='https://placehold.co/200x200/f8faf9/326e54?text=Flower'">
                                </td>
                                <td data-label="Product">
                                    <div style="font-weight:700; color: #333; font-size: 1rem;"><?= htmlspecialchars($row['name']) ?></div>
                                    <?php $liveLink = product_url(['type' => 'flower', 'slug' => $row['slug'] ?? '', 'id' => $row['id']]); ?>
                                ...
                                <td data-label="Stock">
                                    <?php if($row['in_stock']): ?>
                                        <a href="actions/toggle_stock.php?type=flowers&id=<?= $row['id'] ?>&stock=0&csrf_token=<?php echo csrf_token(); ?>" class="badge badge-success badge-clickable">
                                            <i class="fa-solid fa-circle-check"></i> In Stock
                                        </a>
                                    <?php else: ?>
                                        <a href="actions/toggle_stock.php?type=flowers&id=<?= $row['id'] ?>&stock=1&csrf_token=<?php echo csrf_token(); ?>" class="badge badge-danger badge-clickable">
                                ...
                                <td data-label="Actions">
                                    <div style="display: flex; gap: 10px; justify-content: flex-end;">
                                        <a href="edit-flower.php?id=<?= $row['id'] ?>" class="btn-action" title="Edit">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </a>
                                        <a href="actions/delete_flower.php?id=<?= $row['id'] ?>&csrf_token=<?php echo csrf_token(); ?>" 
                                           onclick="return confirm('Permanently delete this flower?')"
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="7" style="text-align:center; padding:100px; color:#bbb;">
                                <i class="fa-solid fa-seedling" style="font-size: 3rem; display:block; margin-bottom:15px; opacity:0.3;"></i>
                                No flowers found in your inventory.
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
