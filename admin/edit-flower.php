...
// One-time Migration for Multiple Categories
$checkCol = $conn->query("SHOW COLUMNS FROM flowers LIKE 'category_id'");
if($checkCol && $checkCol->num_rows > 0) {
    try {
        $conn->query("ALTER TABLE flowers CHANGE category_id category_ids TEXT");
        $res = $conn->query("SELECT id, category_ids FROM flowers");
        while($row = $res->fetch_assoc()) {
            $ids = trim($row['category_ids']);
            if(!empty($ids) && strpos($ids, ',') === false) {
                $newVal = ',' . $ids . ',';
                $stmt = $conn->prepare("UPDATE flowers SET category_ids = ? WHERE id = ?");
                $stmt->bind_param("si", $newVal, $row['id']);
                $stmt->execute();
                $stmt->close();
            }
        }
    } catch(Exception $e) { error_log("Migration failed: " . $e->getMessage()); }
}
...
try {
    $res = mysqli_query($conn, "SELECT * FROM flowers WHERE id=$id");
    $f = mysqli_fetch_assoc($res);
} catch (Exception $e) {
    die("Flower not found or query error.");
}
...
$variants = [];
try {
    $variants_res = mysqli_query($conn, "SELECT * FROM flower_variants WHERE flower_id=$id");
...
// Fetch old Gallery Images from the separate table
$old_gallery_images = [];
try {
    $gal_res = mysqli_query($conn, "SELECT id, image_path FROM flower_images WHERE flower_id=$id");
    if($gal_res) {
        while($g = mysqli_fetch_assoc($gal_res)) {
            $old_gallery_images[] = $g;
        }
    }
} catch (Exception $e) {
    // Table missing or err
}
?>
<!DOCTYPE html>
<html lang="en">
...
<form action="actions/update_flower.php" method="POST" enctype="multipart/form-data">
<?php csrf_field(); ?>
...
<div class="slug-note">Final URL: /flowers/<span id="slugPreview" style="color:var(--primary); font-weight:bold;"><?= htmlspecialchars($f['slug'] ?? 'your-slug'); ?></span></div>
...
<select name="category_ids[]" class="cat-select" multiple style="width:100%; margin-bottom: 20px;" required>
    <?php 
    $current_cat_ids = isset($f['category_ids']) ? explode(',', trim($f['category_ids'], ',')) : [];
    $cat_res = $conn->query("SELECT id, name FROM categories WHERE status=1 ORDER BY name ASC");
    while($cat = $cat_res->fetch_assoc()) {
        $selected = in_array($cat['id'], $current_cat_ids) ? 'selected' : '';
...
<input type="text" name="image_alt" value="<?= htmlspecialchars($f['image_alt'] ?? '') ?>" placeholder="e.g. Premium red rose bouquet for anniversary">
...
<label>Additional Gallery (Max 2 images total)</label>
<div id="galleryPreviewContainer" style="display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 15px;">
    <?php 
    $gallery = [];
    if(!empty($f['images_gallery'])) {
        $gallery = json_decode($f['images_gallery'], true) ?: [];
    }
    
    foreach($gallery as $idx => $img): 
        $gPath = (strpos($img, 'uploads/') === 0) ? "/" . $img : "/uploads/" . $img;
    ?>
        <div class="current-asset-box" style="width: 120px; padding: 5px; position: relative;">
            <img src="<?= $gPath ?>" style="height: 100px; width: 100%; object-fit: cover; margin-bottom: 5px;">
            <a href="actions/delete_flower_image.php?id=<?= $id ?>&type=json&index=<?= $idx ?>&csrf_token=<?= csrf_token() ?>" 
               onclick="return confirm('Remove this image from gallery?')"
               style="position:absolute; top:5px; right:5px; background:rgba(255,255,255,0.9); color:#e74c3c; width:24px; height:24px; border-radius:50%; display:flex; items-center; justify-content:center; text-decoration:none; font-size:12px; border:1px solid #fecaca; box-shadow:0 2px 5px rgba(0,0,0,0.1);">
...
<?php if(count($gallery) < 2): ?>
    ...
    <p style="font-size: 0.7rem; color: #888; margin-top: -15px;">You can upload up to <?= 2 - count($gallery) ?> more image(s).</p>
<?php else: ?>
    <p style="font-size: 0.8rem; color: #e74c3c; font-weight: bold; margin-bottom: 20px;"><i class="fas fa-info-circle"></i> Gallery Full (Limit 2). Delete via database if needed to replace.</p>
<?php endif; ?>
<span id="currentImgCount" style="display:none;"><?= count($gallery) ?></span>
...
<input type="text" name="meta_title" value="<?= htmlspecialchars($f['meta_title'] ?? ''); ?>" placeholder="Flower name + City name">
...
$(document).ready(function() {
    ...
    const currentCountSpan = document.getElementById('currentImgCount');
    const currentImgCount = currentCountSpan ? parseInt(currentCountSpan.innerText) : 0;
    if(galleryInput) {
        galleryInput.addEventListener('change', function() {
            const maxAllowed = 2 - currentImgCount;
            if(this.files.length > maxAllowed) {
                alert("You can only upload " + maxAllowed + " more gallery images (Total limit is 2).");
                this.value = '';
            }
        });
    }
