...
try {
    $res = mysqli_query($conn, "SELECT * FROM cakes WHERE id=$id");
    $f = mysqli_fetch_assoc($res);
} catch (Exception $e) {
    die("Cake not found or query error.");
}
...
$variants = [];
try {
    $variants_res = mysqli_query($conn, "SELECT * FROM cake_variants WHERE cake_id=$id");
...
<div class="card">
<h2><i class="fas fa-edit"></i> Edit Cake</h2>
<p style="color:#666;margin-bottom:30px;">Updating details for: <strong><?= htmlspecialchars($f['name']); ?></strong></p>

<form action="actions/update_cake.php" method="post" enctype="multipart/form-data">
<?php csrf_field(); ?>
...
<div class="slug-note">Final URL: /cakes/<span id="slugPreview" style="color:var(--primary); font-weight:bold;"><?= htmlspecialchars($f['slug'] ?? 'your-slug'); ?></span></div>
...
<input type="text" name="image_alt" value="<?= htmlspecialchars($f['image_alt'] ?? '') ?>" placeholder="e.g. Chocolate truffle cake with fresh berries">
...
<input type="text" name="meta_title" value="<?= htmlspecialchars($f['meta_title'] ?? ''); ?>" placeholder="Cake name + City name">
