...
$checkFaqs = $conn->query("SHOW COLUMNS FROM cakes LIKE 'faqs'");
if($checkFaqs && $checkFaqs->num_rows == 0) {
    try {
        $conn->query("ALTER TABLE cakes ADD COLUMN faqs TEXT DEFAULT NULL");
...
<p style="color:#666;margin-bottom:30px;">Add a fresh cake to your online store.</p>

<form action="actions/add_cake.php" method="POST" enctype="multipart/form-data">
<?php csrf_field(); ?>
...
<div class="slug-note">Final URL: /cakes/<span id="slugPreview" style="color:var(--primary); font-weight:bold;">your-slug</span></div>
...
<textarea name="description" placeholder="Describe the cake..." required style="min-height:120px;"></textarea>

<label>Image Alt Text (SEO)</label>
<input type="text" name="image_alt" placeholder="e.g. Chocolate truffle cake with fresh berries">
...
<input type="text" name="meta_title" placeholder="Cake name + City name">
...
<a href="cakes.php" class="back-link"><i class="fas fa-arrow-left"></i> Back to Cake List</a>
</div>
...
function addFaqRow(q = '', a = '') {
    ...
    qInput.placeholder = 'Question (e.g., Are these cakes eggless?)';
    ...
    aInput.placeholder = 'Answer (e.g., Yes, all our cakes have an eggless option.)';
